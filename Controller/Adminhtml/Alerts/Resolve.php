<?php
/**
 * Alerts Resolve Controller (grid action — redirect)
 *
...
/**
 * Resolve an alert from the grid — sets status to resolved and redirects back.
 */
class Resolve extends Action
{
    ...
    /**
     * Execute: load alert, set resolved, redirect to alerts grid
     *
    ...
    public function execute()
    {
        ...
        try {
            ...
            if ($alert->getData('status') === Alert::STATUS_RESOLVED) {
                $this->messageManager->addNoticeMessage(__('Alert #%1 is already resolved.', $alertId));
                return $resultRedirect->setPath('stockpredict/alerts/index');
            }
            ...
            $this->messageManager->addSuccessMessage(
                __('Alert #%1 (%2) has been resolved.', $alertId, $alert->getData('sku'))
            );

        } catch (\Exception $e) {
            $this->messageManager->addErrorMessage(__('Could not resolve alert: %1', $e->getMessage()));
        }
